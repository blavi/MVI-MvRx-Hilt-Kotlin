...
interface ApiEndpoints {
    ...
    fun fetchTrendingMovies(@Path(MEDIA_TYPE) media_type: String = NetworkConstants.MEDIA_TYPE_MOVIES,
                            @Path(TIME_WINDOW) time_window: String = NetworkConstants.TIME_WINDOW_WEEK,
                            @Query(PAGE) page: Int,
                            @Query(API_KEY) apiKey: String = BuildConfig.TMDB_API_KEY): Single<TMDBResponse>
    ...
    fun fetchComingSoonTVShows(@Query(API_KEY) apiKey: String  = BuildConfig.TMDB_API_KEY,
                               @Query(TV_SHOW_RELEASE_YEAR) year: Int,
                               @Query(SORT_BY) sortByValue: String,
                               @Query(PAGE) page: Int): Single<TMDBResponse>
}